...
const Header = () => {
  ...
  return (
    ...
    {/* Desktop Navigation */}
    <nav className="hidden md:flex items-center space-x-6 lg:space-x-10 flex-wrap">
      {navLinks.map((link, index) => (
        <a
          key={index}
          href={link.href}
          className="relative text-black font-medium text-sm sm:text-base md:text-lg lg:text-xl group hover:text-blue-500 transition"
        >
          {link.name}
  ...
  <AnimatePresence>
    {menuOpen && (
      <motion.div
        ...
        className="md:hidden mt-4 bg-white rounded-xl shadow-lg p-4 sm:p-6 flex flex-col items-center gap-4"
      >
        {navLinks.map((link, index) => (
          <a
            key={index}
            href={link.href}
            className="text-black text-base sm:text-lg font-medium hover:text-blue-500 transition"
